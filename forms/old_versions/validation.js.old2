...
var canSubmit = function() {
  for (var key of keys) {
    if (
      ...
      paragraph_discard.checked == true &&
      $("#discard_reason").val() == ""
    ) {
      return false;
    }
...
var show = function() {
  ...
  if (canSubmit()) {
    activate_cbutton(submit);
  } else {
    deactivate_cbutton(submit);
  }

  if (canSubmitStay()) {
    activate_cbutton(submitStay);
  } else {
    deactivate_cbutton(submitStay);
  }
...
//copi doi to clipboard
$("#copy_doi_btn").click(CopyToClipboard);
...
//remove all annotations for selected key
$("#remove").click(function() {
  remove_all_annotations();
  show();
});

//manage 'no reaction' option
$('#noReaction').click(function() {
   old_key = key;
   if (this.checked == true) {
    for (var key_change of keys) {
       key = key_change;
       remove_all_annotations();
       noVal[key].prop("checked", true);
       show();
    }
    $("#paragraph_discard").prop("checked", false);
    $("#paragraph_discard").attr("disabled", true);
  } else {
    $("#paragraph_discard").removeAttr("disabled");
  }
   key = old_key;
   show();
});

//manage paragraph discard
$('#discard_reason').change(function() {
  show();
});

//manage paragraph discard reason
$('#paragraph_discard').click(function() {
   old_key = key;
   if (this.checked == true) {
     for (var key_change of keys) {
       key = key_change;
     ...
     $("#discard_reason").val("");
     $("#discard_reason").show();
     $("#noReaction").prop("checked", false);
     $("#noReaction").attr("disabled", true);
     deactivate_cbutton(submitStay);
   } else {
     $("#discard_reason").hide();
     $("#noReaction").removeAttr("disabled");
     activate_cbutton(submitStay);
     show();
   }
   key = old_key;
...
function CopyToClipboard(element) {
  ...
  dummy.setAttribute('value', cveid_text)
  dummy.select();
...
spans.hide();
var cveid_text = "https://pubs.acs.org/doi/".concat(cveid.text().split("-")[0]);
document.querySelector(".results").innerHTML = cveid_text;
document.querySelector(".results").setAttribute("href", cveid_text);
document.querySelector(".results").setAttribute("target", "_blank");
$("#discard_reason").hide();
